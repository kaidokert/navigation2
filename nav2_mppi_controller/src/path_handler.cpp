  ...
  const geometry_msgs::msg::PoseStamped & robot_pose)
{
      ...
      ghost_heading, ghost_heading_variance))
  {
    ...
    if (appended > 0) {
      RCLCPP_INFO(logger_,
        "[GHOST_PATH] appended %zu pts heading=%.3frad(%.0fdeg) var=%.5f "
        "real=%.4fm total=%.4fm cusp=%.4fm",
        appended, ghost_heading, ghost_heading * 180.0 / M_PI,
        ghost_heading_variance,
  ...
  double & seed_arc_length) const
{
  ...
  if (transformed_plan.poses.size() == 1) {
    ...
    // Peek at the full plan to detect reverse travel direction.
    // Use global_plan_ (unpruned) rather than global_plan_up_to_inversion_
    // (which may be pruned to 1 point after handoff + closest-point prune).
    if (global_plan_.poses.size() >= 2) {
      const auto & p0 = global_plan_.poses[0].pose.position;
      const auto & p1 = global_plan_.poses[1].pose.position;
